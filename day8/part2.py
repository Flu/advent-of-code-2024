...
def get_antinodes(antenna1, antenna2):
    """Return a list of tuples with the coordinates of all antinodes formed by antenna1 and antenna2, relative to antenna1.
        This function assumes that antenna1 and antenna2 have the same frequency."""
    result = []
    # Antenna 1 itself is also an antinode now
    result.append(antenna1)

    # Calculate the first antinode, as we did in part 1
    x1 = antenna1[0] - (antenna2[0]-antenna1[0])
    y1 = antenna1[1] - (antenna2[1]-antenna1[1])
    result.append((x1,y1))

    # Then go on with the same offsets and add more antinodes until the coordinates are no longer valid
    while valid_coords(data, x1, y1):
        x1 -= (antenna2[0]-antenna1[0])
        y1 -= (antenna2[1]-antenna1[1])
        result.append((x1,y1))
    return result
...
for frequency, antenna_list in frequency_location.items():
    ...
    for antenna1 in antenna_list:
        for antenna2 in antenna_list:
            # We already add the antenna itself as an antinode in get_antinodes()
